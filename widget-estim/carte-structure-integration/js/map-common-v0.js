...
(function ($) {
	...
	*/
	...
	function getInfoItem( item ) {
		...
		if ( item.urlDetailItem ) {
			content += '<a target="_blank" href="' + urlDetailItem + '">' + item.title + '</a>';
		} else {
	...
	var searchItemsBag = (function(){
		...
		return {
			addItemBag : function (item, marker) {
	...
	zf.addItems = function (points) {
		for (var i in points) {
			...
			for (var i=0; i<items.length; i++) {
				searchItemsBag.addItemBag(items[i], marker);				
			}
